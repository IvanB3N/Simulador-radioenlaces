    ...
    #Funcion principal que para calcular los datos del radio enlace
     def calculos(self):
        try: 
            ...
            anchoBanda = self.anchoBanda.text() #Ancho de banda [GHz]
            potTransmitida = self.potTr.text() #Potenci transmitida mmW
            ...
            if isfloat(f) == False or float(f)<=0:
                QApplication.restoreOverrideCursor() #Detener cargando en el cursor
                QMessageBox.about(self, "Error", "Reingrese frecuencia")
                return
            if isfloat(potTransmitida) == False or float(potTransmitida)<=0:
                QApplication.restoreOverrideCursor() #Detener cargando en el cursor
                QMessageBox.about(self, "Error", "Reingrese potencia transmitida")
                return  
            if isfloat(anchoBanda) == False or float(anchoBanda)<=0:
                QApplication.restoreOverrideCursor() #Detener cargando en el cursor
                QMessageBox.about(self, "Error", "Reingrese ancho de banda")
                return
            if isfloat(nElement) == False or float(nElement)<=0:
                QApplication.restoreOverrideCursor() #Detener cargando en el cursor
                QMessageBox.about(self, "Error", "Reingrese numero elementos")
                return 
            if isfloat(gainR) == False or float(gainR)<=0:
                QApplication.restoreOverrideCursor() #Detener cargando en el cursor
                QMessageBox.about(self, "Error", "Reingrese ganancia por elemento")
                return  

            f = float(f) #Conversion String -> Float
            anchoBanda = float(anchoBanda) #Conversion String -> Float
            potTransmitida = float(potTransmitida) #Conversion String -> Float
            ...
            if estado ==1:
                if isfloat(htorre) == False or float(htorre)<=0:
                    QApplication.restoreOverrideCursor() #Detener cargando en el cursor
                    QMessageBox.about(self, "Error", "Reingrese altura torre fija")
                    return
                htorre = float(htorre)
            ...
            [Pr_v, Pr_h, PN_dbm] = atenuaciones(f,distancia,gmax_db,gmax_db,por,zon,lat,asn,anchoBanda,potTransmitida)
...
SNR: {round((Pr_v) - PN_dbm,2)} dBm
            ...
            QApplication.restoreOverrideCursor() #Detener cargando en el cursor
            QMessageBox.about(self, "Error", "Uno o más campos están vacíos o mal ingresados.")
            return
