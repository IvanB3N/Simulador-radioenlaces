...
def atenuacion_gases(f, T, p, e, distancia):
    """
    ...
    p : TYPE float
        DESCRIPTION. Presión del aire seco (hPa)
    e : TYPE float
        DESCRIPTION. Presión parcial del vapor de agua (hPa)
    ...
    -------
    A_gases : TYPE float
        DESCRIPTION. atenuacion debida a los gases [dB]

    """
    
    # In[Calculo de atenuacion debida a Oxigeno]
    # Datos espectroscópicos para la atenuación del oxígeno
    oxigeno = {1:[50.474214,0.975,9.651,6.690,0.0,2.566,6.850],
    ...
    theta = 300/T
    anchura_banda = 5.6*10**(-4)*(p+e)*theta**(0.8) #anchura de banda del espectro de Debye
    
    Nd = f*p*theta**2*((6.14*10**(-5))/(anchura_banda*(1+(f/anchura_banda)**2))) + (1.4*10**(-12)*p*theta**1.5)/(1+(1.9*10**(-5)*f**1.5)) # espectro continuo de aire seco
    
    N_oxi = 0.0 #  parte imaginaria del valor complejo de la refractividad
    for i in range(1, (len(oxigeno) + 1)):
        delta = (oxigeno[i][5] + oxigeno[i][6])*10**(-4)*(p + e)*theta**0.8 # factor de corrección por oxigeno
        
        df = oxigeno[i][3]*10**(-4)*(p*theta**(0.8-oxigeno[i][4]) + 1.1*e*theta) # anchura de la raya
        
        df = np.sqrt(df**2 + (2.25*10**(-6))) # anchura de la raya modificado
        
        # factor de forma de raya
        Fi = (f/oxigeno[i][0])*((df-delta*(oxigeno[i][0]-f))/((oxigeno[i][0]-f)**2+df**2) + (df-delta*(oxigeno[i][0]+f))/((oxigeno[i][0]+f)**2+df**2))
        
        # valor de la i-ésima raya del oxígeno
        Si = oxigeno[i][1]*10**(-7)*p*theta**3*np.exp(oxigeno[i][2]*(1-theta))
        
        N_oxi = N_oxi + Si*Fi + Nd
    
    # In[Calculo de atenuacion debida al vapor de agua]
    # Datos espectroscópicos para la atenuación del vapor de agua
    vapor_agua = {1:[22.235080,0.1079,2.144,26.38,0.76,5.087,1.00],
    ...
    Fi = 0
    Si = 0
    for i in range(1, len(vapor_agua)+1):
        df = vapor_agua[i][3]*10**(-4)*(p*theta**vapor_agua[i][4] + vapor_agua[i][5]*e*theta**(vapor_agua[i][6]))
        
        df = 0.5335*df + np.sqrt(0.217*df**2 + ((2.1316*10**(-12)*vapor_agua[i][0])/theta))
        # factor de forma de raya
        Fi = (f/vapor_agua[i][0])*((df-delta*(vapor_agua[i][0]-f))/((vapor_agua[i][0]-f)**2+df**2) + (df-delta*(vapor_agua[i][0]+f))/((vapor_agua[i][0]+f)**2+df**2))
        
        Si = vapor_agua[i][1]*10**(-1)*e*theta**3.5*np.exp(vapor_agua[i][2]*(1-theta))
        
        N_vapor = N_vapor + Si*Fi
    
    # In[Atenuacion debida a gases oxigeno + vapor de agua]
    # atenuación específica
    y_gases = 0.1820*f*(N_oxi + N_vapor) #[dB/km]
    
    # Atenuacion debida a gases
    A_gases = y_gases*distancia
